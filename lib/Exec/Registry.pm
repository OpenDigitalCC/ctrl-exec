...
use Fcntl     qw(:flock);
use Carp      qw(croak);
use Exec::FileUtil qw(slurp write_atomic);
...
#       normalises to 'hostname'. See resolve_target.
...
sub register_agent {
    ...
    write_atomic("$dir/$hostname.json", encode_json($record));
}
...
# The atomic temp+rename write gives torn-free reads, but two
...
sub update_agent_tags {
    ...
    return _locked($dir, sub {
        my $record = eval { decode_json(slurp($path)) } or return 0;
...
sub list_agents {
    ...
    while (my $f = readdir $dh) {
        next unless $f =~ /^(.+)\.json$/;
        my $data = eval { decode_json(slurp("$dir/$f")) };
        next if $@;
        push @agents, $data;
    }
    closedir $dh;
...
# ("address" or "address:port") dispatch should use. Appends the agent's stored operational port when it differs from the
...
sub get_agent {
    ...
    return eval { decode_json(slurp($path)) };
}
...
sub remove_agent {
    ...
    my $record = eval { decode_json(slurp($path)) } // {};
...
sub edit_agent {
    ...
    return _locked($dir, sub {
        ...
        if (defined $new && length $new && $new ne $hostname) {
            ...
            $record->{hostname} = $new;
            write_atomic("$dir/$new.json", encode_json($record));
